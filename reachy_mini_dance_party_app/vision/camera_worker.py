...
Lifted from `reachy_mini_conversation_app.camera_worker` (v0.6.1) and adapted:
the HeadTracker Protocol that previously lived in
`reachy_mini_conversation_app.vision.head_tracking` is inlined below so this
module is self-contained inside the dance-party app. For the dance-party use
case the worker is typically constructed with `head_tracker=None` and the
face-tracking secondary offsets are produced by `vision.face_tracker` on top
of `get_latest_frame()`.
"""
...
import logging
import threading
from typing import List, Protocol, SupportsFloat, Tuple, TypeAlias
