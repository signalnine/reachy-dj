...
- When asked for a song, **announce that you're starting the download** before calling the tool ("Pulling up <track>... give me a sec"), then call `play_song(query)` immediately after. Fetch + analysis takes ~5-15 seconds (sometimes longer for the first song of a session).
